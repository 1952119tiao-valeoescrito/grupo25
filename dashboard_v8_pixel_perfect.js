...
import { Trophy, RefreshCw, ChevronRight, Loader2, LogOut, Wallet, Scale, HelpCircle, Mail, Phone } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center font-elite">
          ...
          <div className="flex items-center gap-4">
             <span className="text-[9px] font-black text-yellow-500 border border-yellow-500/20 px-3 py-1 rounded-full uppercase italic tracking-tighter">Olá, {user.nome.split(' ')[0]}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-16">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
             ...
             <p className="text-[10px] text-white/30 text-center mt-6 uppercase font-bold italic tracking-widest italic">Identificado: {user.nome}</p>
          </div>
        ...
        {/* --- 2. MEIO: PORTAL DE ACESSO E CRÉDITO (DO PRINT 2) --- */}
        ...
        <section className="mb-20">
           ...
           <p className="text-cyan-400 font-bold uppercase tracking-[0.4em] text-[10px] font-elite mb-12">Nossa produção 100% blockchain</p>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12 max-w-5xl mx-auto px-4">
          <div className="bg-[#0f172a] border border-cyan-500/20 p-10 rounded-[3rem] shadow-2xl text-center">
             ...
             <p className="text-[10px] text-white uppercase font-bold mb-8 tracking-widest italic">Ganha com até 1 ponto</p>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/inter-bet')} className="inline-block w-full py-5 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs transition-all hover:scale-105">Acessar Site</button>
          </div>
          <div className="bg-[#0f172a] border border-cyan-500/20 p-10 rounded-[3.5rem] shadow-2xl text-center">
             <h3 className="text-xl text-cyan-400 mb-2 font-black uppercase italic font-elite">QUINA-BET</h3>
             <p className="text-[10px] text-white uppercase font-bold mb-8 tracking-widest italic">Sorteios Semanais</p>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/quina-bet')} className="inline-block w-full py-5 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs transition-all hover:scale-105">Acessar Site</button>
          </div>
        </div>

        {/* --- 5. RODAPÉ FINAL: CONTATO E COPYRIGHT --- */}
...
Write-Host "🚀 DASHBOARD 100% CONCLUÍDO! Ordem exata dos blocos restaurada." -ForegroundColor Cyan