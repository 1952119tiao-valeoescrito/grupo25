...
export default function DashboardElite() {
  ...
  const gerarMalha = () => {
    ...
    setMatriz(linhas);
    setQrCode(""); // Reseta o Pix se trocar os números
  };
  ...
  const handleGerarPix = async () => {
    ...
    try {
      const res = await fetch('/api/pix/create', {
        ...
        body: JSON.stringify({ email: user.email, prognosticos: matriz.flat(), rodadaId: 1 })
      });
      ...
      if(data.qrCode) {
        ...
        alert("Erro: " + (data.error || "Verifique seu token na Vercel"));
      }
  ...
  const handleConfirmar = () => {
    ...
    // REDIRECIONA E LIMPA A MATRIX (REGRA DE MESA LIMPA)
    setMatriz([]);
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <div className="bg-[#0d1117]/80 border border-cyan-500/20 p-10 md:p-14 rounded-[4rem] mb-12 max-w-4xl mx-auto shadow-2xl backdrop-blur-md">
           ...
           <div className="grid md:grid-cols-2 gap-10 text-center">
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2 text-center font-elite">E-mail Suporte</p><p className="text-sm font-bold text-center text-white italic tracking-tighter">suporte@blockchain-betbrasil.io</p></div>
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2 text-center font-elite">WhatsApp Oficial</p><p className="text-sm font-bold text-center text-white italic tracking-tighter">+55 (21) 99352-7957</p></div>
           </div>
        </div>
...
console.log("✅ DASHBOARD 100% OPERACIONAL E FIEL RESTAURADO!");