...
import { Trophy, RefreshCw, ChevronRight, Loader2, Wallet, LogOut, Scale, HelpCircle } from 'lucide-react';
  ...
  const handlePagamento = async () => {
    ...
    try {
      const res = await fetch('/api/pix/create', {
        ...
        body: JSON.stringify({ cpf: user.email.split('@')[0], email: user.email, prognosticos: matriz.flat(), rodadaId: 1 })
      });
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden selection:bg-cyan-500/30">
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-12 text-center relative z-10">
        ...
        <section className="mb-12">
          ...
          <div style={{fontFamily:'Orbitron'}} className="text-5xl md:text-8xl mb-4 drop-shadow-[0_0_15px_rgba(34,211,238,0.8)] text-white">{timer}</div>
          <p className="text-[10px] text-yellow-500 uppercase font-bold tracking-widest italic text-center">Sábado às 20:00hrs</p>
        </section>

        {/* PAINEL CENTRAL MATRIX (IDÊNTICO) */}
        <div className="bg-[#0f172a]/85 border border-cyan-500/20 p-6 md:p-12 rounded-[3rem] shadow-2xl mb-12">
          <h2 className="text-yellow-500 font-bold text-xs uppercase mb-8 tracking-widest">Sua Malha de Coordenadas Matrix 5x5</h2>
          <div className="bg-black/60 border border-slate-800 rounded-[1.5rem] p-4 md:p-10 mb-10 max-w-2xl mx-auto">
          ...
          {!qrCode ? (
            <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
              ...
              <button onClick={handlePagamento} disabled={loading} className="flex-1 bg-[#ea580c] hover:bg-orange-500 text-white py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg flex justify-center items-center gap-2">
          ...
          <p className="text-[11px] text-white mt-8 font-bold italic uppercase tracking-widest italic animate-pulse">
            IDENTIFICADO: {user.nome}
          </p>
        </div>

        {/* SEÇÃO LADO A LADO: LUCRO + COTAS LEGAIS */}
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto mb-20">
            {/* AFILIADOS (IDÊNTICO) */}
            <div className="bg-[#0f172a]/85 border border-amber-500/30 p-8 rounded-[2rem] text-left relative overflow-hidden shadow-2xl">
                ...
                <div className="flex justify-between items-end mb-6">
                    ...
                    <button className="bg-amber-600 hover:bg-amber-500 text-white text-[10px] font-black px-5 py-2.5 rounded-xl transition-all">SACAR VIA PIX</button>
                </div>
            ...
            {/* NOVA SEÇÃO: COTAS LEGAIS (CONFORME CONTRATO) */}
            <div className="bg-[#0f172a]/85 border border-white/10 p-8 rounded-[2rem] text-left shadow-2xl">
                <h3 className="text-[11px] text-yellow-500 mb-6 uppercase font-black tracking-widest italic">⚖️ Transparência (Lei 13.756)</h3>
                <div className="space-y-4 text-[10px] font-bold text-slate-400 uppercase">
                    <div className="flex justify-between border-b border-white/5 pb-2"><span>Prêmio Bruto (43,35%)</span><span className="text-white">R$ 0,00</span></div>
                ...
                <p className="mt-6 text-[8px] text-slate-600 italic leading-relaxed">Todos os repasses são auditados via Blockchain Base Mainnet.</p>
            </div>
        </div>

        {/* CARDS PRODUTOS (IDÊNTICOS) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 max-w-[1000px] mx-auto px-2">
          <div className="bg-[#0f172a]/85 border border-cyan-500/20 p-10 rounded-[3rem] text-center">
             <h3 className="text-xl text-yellow-500 mb-2 font-black uppercase">INTER-BET</h3>
             <p className="text-[10px] text-white uppercase font-bold mb-8">GANHA COM ATÉ 1 PONTO</p>
             <a href="https://blockchain-betbrasil.io/pt/inter-bet" target="_blank" className="inline-block w-full py-4 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs">Acessar Site</a>
          </div>
          <div className="bg-[#0f172a]/85 border border-cyan-500/20 p-10 rounded-[3rem] text-center">
             <h3 className="text-xl text-cyan-400 mb-2 font-black uppercase">QUINA-BET</h3>
             <p className="text-[10px] text-white uppercase font-bold mb-8">SORTEIOS SEMANAIS</p>
             <a href="https://blockchain-betbrasil.io/pt/quina-bet" target="_blank" className="inline-block w-full py-4 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs">Acessar Site</a>
          </div>
        </div>
        ...
        <div className="bg-[#0f172a]/70 border border-cyan-500/20 p-12 rounded-[4rem] mb-20 max-w-4xl mx-auto">
           ...
           <div className="grid md:grid-cols-2 gap-10">
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2">E-mail</p><p className="text-sm font-bold">suporte@blockchain-betbrasil.io</p></div>
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2">WhatsApp</p><p className="text-sm font-bold">+55 (21) 99352-7957</p></div>
           </div>
        </div>

        {/* FOOTER COM PORTA SECRETA (IDÊNTICO) */}
        <footer className="py-16 border-t border-white/5 text-center">
           <p onClick={()=>{setClickCount(c=>c+1); if(clickCount>=4) router.push('/admin/central')}} className="text-[10px] text-white uppercase tracking-[0.5em] font-black cursor-pointer hover:text-yellow-500 transition-all antialiased select-none">
      ...
      <div className="fixed bottom-6 right-6 w-14 h-14 bg-cyan-600 rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.5)] cursor-pointer hover:scale-110 transition-all z-[100]">
         <HelpCircle size={28} />
      </div>
...
console.log("✅ Dashboard Original Restaurado com Cotas Oficiais!");