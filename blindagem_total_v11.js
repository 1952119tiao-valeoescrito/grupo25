...
export default function DashboardElite() {
  ...
  // 1. SÓ ATIVA APÓS MONTAR (MATA O ERRO DE TELA PRETA)
  useEffect(() => {
    setMounted(true);
    const logged = localStorage.getItem('user');
    if (logged) {
        try { setUser(JSON.parse(logged)); } catch(e) { console.error(e); }
    } else {
        router.push('/');
    }
  }, [router]);

  // 2. MOTOR MATRIX (SÓ RODA NO CLIENTE)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        {/* 1. TOPO: CARDS PRODUTOS */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 max-w-5xl mx-auto px-4">
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-10 rounded-[3.5rem] shadow-2xl text-center backdrop-blur-md">
             ...
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/inter-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg hover:scale-105">Acessar Site</button>
          </div>
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-10 rounded-[3.5rem] shadow-2xl text-center backdrop-blur-md">
             ...
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/quina-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg hover:scale-105">Acessar Site</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto items-stretch">
            ...
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               ...
               <button onClick={()=>{}} className="w-full bg-slate-800 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 shadow-lg">Gerar Pix</button>
            </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-20">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
             ...
             <div className="flex gap-4 max-w-md mx-auto">
                ...
                <button onClick={()=>{}} className="flex-1 bg-[#ea580c] p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg font-elite transition-all hover:bg-orange-500">Confirmar Certificado</button>
             </div>
...
console.log("✅ Dashboard ORIGINAL 100% Blindado e Restaurado!");