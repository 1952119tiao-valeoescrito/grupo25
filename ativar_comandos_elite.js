...
export default function DashboardElite() {
  ...
  const [timer, setTimer] = useState("05:01:55:27");
  ...
  // FUNÇÃO DO BOTÃO LARANJA "CONFIRMAR CERTIFICADO"
  const handleConfirmarCertificado = () => {
    if(!qrCode) return alert("Gere o Pix primeiro!");
    // Redireciona para o bilhete salvo
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 max-w-5xl mx-auto px-4">
          <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-8 md:p-10 rounded-[3.5rem] shadow-2xl text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto items-stretch">
            ...
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               ...
               <div className="bg-white p-4 rounded-3xl mb-8 shadow-inner flex items-center justify-center">
                 {!qrCode ? (
                    <div className="w-32 h-32 bg-slate-100 flex items-center justify-center rounded-xl">
                        {loading ? <Loader2 className="animate-spin text-cyan-500" /> : <div className="text-slate-300 text-[8px] uppercase font-bold">Aguardando...</div>}
                    </div>
                 ) : <QRCodeSVG value={qrCode} size={128} />}
               </div>
               <button onClick={handleGerarPix} disabled={loading} className="w-full bg-slate-800 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 transition-all hover:bg-slate-700">
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-20">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
             ...
             <div className="flex gap-4 max-w-md mx-auto">
                ...
                <button onClick={handleConfirmarCertificado} className="flex-1 bg-[#ea580c] p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg font-elite transition-all hover:bg-orange-500">Confirmar Certificado</button>
             </div>
...
console.log("✅ SISTEMA TOTALMENTE ATIVADO! Os botões agora têm alma.");