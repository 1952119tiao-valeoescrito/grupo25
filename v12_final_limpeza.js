...
export default function DashboardElite() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillText("MATRIX G25 ACTIVE", Math.random()*canvas.width, Math.random()*canvas.height);
  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      if(data.qrCode) {
        ...
        setMatriz([]); // LIMPA A MATRIX PARA O PRÓXIMO USUÁRIO
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30 text-center">
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center">
          <h1 className="text-white text-sm md:text-xl font-black uppercase italic tracking-tighter">MIMOSINHA<span className="text-cyan-400">G25</span></h1>
      ...
      <main className="max-w-6xl mx-auto px-4 py-10 relative z-10">
        ...
        <div className="max-w-4xl mx-auto bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl mb-12">
            ...
            <div className="flex gap-4 max-w-md mx-auto">
                <button onClick={gerarMalha} className="flex-1 bg-slate-800 p-4 rounded-2xl font-black text-[10px] uppercase border border-white/5 transition-all hover:bg-slate-700">Trocar Coordenadas</button>
                <button onClick={handleConfirmar} disabled={loading || matriz.length === 0} className="flex-1 bg-[#ea580c] p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg transition-all hover:bg-orange-500 flex justify-center items-center gap-2">
