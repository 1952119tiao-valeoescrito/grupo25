  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      if(data.qrCode) {
        ...
        setMatriz([]); // Limpa a matriz na tela
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <main className="max-w-[1100px] mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <section className="mb-10">
          ...
          <p className="text-[10px] text-yellow-500 uppercase font-bold tracking-widest italic mb-2">Sábado às 20:00hrs</p>
        ...
        <div className="grid lg:grid-cols-3 gap-10 items-start text-left mb-16">
          ...
          {/* COLUNA ESQUERDA: MALHA MATRIX */}
          <div className="lg:col-span-2 bg-[#0f172a]/95 border border-cyan-500/30 p-8 md:p-12 rounded-[3.5rem] shadow-2xl">
             ...
             <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                <button onClick={gerarMalha} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-4 rounded-2xl font-black uppercase text-[10px] transition-all border border-white/5 font-elite">Gerar Coordenadas</button>
                <button onClick={handleConfirmar} disabled={loading || matriz.length === 0} className="flex-1 bg-[#ea580c] hover:bg-orange-500 text-white py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg flex justify-center items-center gap-2 font-elite">
                   {loading ? <Loader2 className="animate-spin" size={14}/> : "Confirmar Certificado"}
                </button>
             </div>
             <p className="text-[10px] text-white/30 text-center mt-8 font-bold uppercase tracking-widest italic animate-pulse">IDENTIFICADO: {user.nome}</p>
          </div>

          {/* COLUNA DIREITA: LUCRO, RANKING E LEI */}
          <div className="space-y-6">
            <div className="bg-[#0f172a]/95 border border-amber-500/30 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden">
               ...
               <h3 className="text-[11px] text-amber-500 mb-6 font-black uppercase tracking-widest italic font-elite">💰 Meu Lucro</h3>
               ...
               <button className="w-full bg-[#ea580c] hover:bg-orange-500 text-white p-4 rounded-xl text-[10px] font-black uppercase mt-6 shadow-lg transition-all font-elite">SACAR VIA PIX</button>
            </div>

            <div className="bg-[#0f172a]/95 border border-emerald-500/30 p-8 rounded-[2.5rem] shadow-2xl">
            ...
            <div className="bg-[#0f172a]/95 border border-white/5 p-8 rounded-[2.5rem] shadow-2xl text-left">
        ...
        {/* PRODUTOS NO RODAPÉ */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 my-20 max-w-5xl mx-auto px-4">
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-10 rounded-[3.5rem] shadow-2xl text-center">
             ...
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/inter-bet')} className="inline-block w-full py-4 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs transition-all hover:scale-105">Acessar Site</button>
          </div>
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-10 rounded-[3.5rem] shadow-2xl text-center">
             ...
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/quina-bet')} className="inline-block w-full py-4 bg-cyan-500 text-black rounded-[2rem] font-black uppercase text-xs transition-all hover:scale-105">Acessar Site</button>
          </div>
        </div>
