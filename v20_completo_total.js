...
import { Trophy, RefreshCw, ChevronRight, Loader2, LogOut, Wallet, Scale, HelpCircle, Mail, MessageSquare, Terminal, Database } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center font-elite">
          ...
          <nav className="hidden md:flex gap-6 text-[9px] font-bold uppercase tracking-widest text-slate-400">
             ...
             <button onClick={()=>{localStorage.clear(); router.push('/')}} className="text-red-500 font-black uppercase">Sair</button>
          </nav>
          <div className="flex items-center gap-4">
             <p className="text-[10px] font-bold text-yellow-500 uppercase border border-yellow-500/20 px-4 py-1.5 rounded-full bg-yellow-500/10 italic">Olá, {user.nome.split(' ')[0]}</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        {/* 1. MALHA MATRIX E COLUNA DIREITA */}
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-16">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
             ...
             <div className="bg-black/90 border border-slate-800 rounded-[2.5rem] p-4 md:p-10 mb-8 shadow-inner">
             ...
             <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                <button onClick={gerarMalha} className="flex-1 bg-slate-800 p-4 rounded-2xl font-black text-[10px] uppercase border border-white/5 font-elite">Gerar Coordenadas</button>
                <button onClick={handleConfirmar} disabled={loading || matriz.length === 0} className="flex-1 bg-[#ea580c] p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg font-elite flex justify-center items-center gap-2">
          ...
          <div className="space-y-6">
            <div className="bg-[#0f172a] border border-amber-500/30 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden">
               ...
               <button className="w-full bg-[#ea580c] p-4 rounded-xl text-[10px] font-black uppercase mt-6 shadow-lg">SACAR VIA PIX</button>
            </div>
            <div className="bg-[#0f172a] border border-emerald-500/30 p-8 rounded-[2.5rem] shadow-2xl text-center">
               ...
               <p className="text-slate-500 italic text-[11px] uppercase tracking-tighter animate-pulse">Sincronizando competidores...</p>
            </div>
            <div className="bg-[#0d1117] border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
               <h3 className="text-yellow-500 mb-6 uppercase font-bold text-[10px] tracking-widest italic font-elite">⚖️ Transparência Legal</h3>
        ...
        {/* 2. LOGIN E PIX (LADO A LADO) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto">
            <div className="bg-[#0f172a] border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl text-left backdrop-blur-md">
            ...
            <div className="bg-[#0f172a] border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               ...
               <button onClick={handleGerarPix} className="w-full bg-slate-800 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 shadow-lg">Gerar Pix</button>
            </div>
        </div>
        ...
        {/* 4. CARDS PRODUTOS */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20 max-w-5xl mx-auto px-4">
          <div className="bg-[#0f172a] border border-cyan-500/20 p-10 rounded-[3rem] shadow-2xl text-center">
             <h3 className="text-xl text-yellow-500 mb-2 font-black uppercase italic font-elite tracking-widest">INTER-BET</h3>
             <p className="text-[10px] text-slate-400 mb-8 font-bold">Ganha com até 1 ponto</p>
          ...
          <div className="bg-[#0f172a] border border-cyan-500/20 p-10 rounded-[3rem] shadow-2xl text-center">
             <h3 className="text-xl text-cyan-400 mb-2 font-black uppercase italic font-elite tracking-widest">QUINA-BET</h3>
             <p className="text-[10px] text-slate-400 mb-8 font-bold">Sorteios Semanais</p>
        ...
        <div className="bg-[#0d1117]/70 border border-cyan-500/20 p-10 md:p-12 rounded-[4rem] mb-12 max-w-4xl mx-auto shadow-2xl backdrop-blur-md">
           ...
           <div className="grid md:grid-cols-2 gap-10 text-center">
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2 text-center font-elite tracking-widest">E-mail Suporte</p><p className="text-sm font-bold text-center text-white italic">suporte@blockchain-betbrasil.io</p></div>
              <div><p className="text-[11px] text-slate-500 uppercase font-black mb-2 text-center font-elite tracking-widest">WhatsApp Oficial</p><p className="text-sm font-bold text-center text-white italic">+55 (21) 99352-7957</p></div>
           </div>
        </div>

        <footer className="py-20 border-t border-white/5 opacity-30 text-center relative z-[200]">
           <p onClick={()=>{setClickCount(c=>c+1); if(clickCount>=4) router.push('/admin/central')}} className="text-[9px] font-black uppercase tracking-[0.5em] italic cursor-pointer select-none antialiased">
...
console.log("✅ DASHBOARD REESTRUTURADO 100%! Tudo restaurado.");