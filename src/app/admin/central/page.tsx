...
import { ethers } from 'ethers'; // 🚀 IMPORTADO PARA WEB3
...
import ABI from '@/lib/abi.json'; // 🚀 CERTIFIQUE-SE QUE ESTE ARQUIVO EXISTE EM /src/lib/abi.json
...
export default function PainelAdminMaster() {
  ...
  const [stats, setStats] = useState<any>({ arrecadacao: 'R$ 0,00', totalPagos: 0, arrecadacaoPura: 0 });
  ...
  // --- 🔗 FUNÇÕES WEB3 (O MOTOR DO ARQUIVO SOLTO) ---
  const handleSorteioOficialBlockchain = async () => {
    ...
    try {
      ...
      // O contrato espera o valor em Wei ou escala. Usamos a arrecadação pura do banco
      const tx = await contract.realizarSorteioOficial(stats.totalPagos * 1000); // Exemplo de escala
      
      alert("Comando enviado à Base Mainnet! Aguardando mineração...");
      await tx.wait();
      alert("✅ Sorteio Blockchain Iniciado com Sucesso via Chainlink VRF!");
    } catch (e: any) {
  ...
  const iniciarNovaRodada = async () => {
    ...
    setLoading(true);
    const res = await fetch('/api/admin/rounds/next', { method: 'POST', body: JSON.stringify({ secret }) });
  ...
  const dispararContingencia = async () => {
    if (!secret) return alert("Chave Admin Requerida!");
    setLoading(true);
    const res = await fetch('/api/admin/contingencia', { method: 'POST', body: JSON.stringify({ secret }) });
  ...
  const baixarPremio = async (id: string) => {
    ...
    const res = await fetch('/api/admin/payout', { method: 'POST', body: JSON.stringify({ ticketId: id, secret }) });
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        ...
        {/* ABA SORTEIO (AGORA COM BLOCKCHAIN ATIVO) */}
        {tab === 'sorteio' && (
          <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4">
             ...
             <div className="bg-slate-900/80 p-10 rounded-[3rem] border border-red-500/20">
                ...
                <input type="password" placeholder="ADMIN KEY" value={secret} onChange={e=>setSecret(e.target.value)} className="w-full bg-black border border-slate-800 p-4 rounded-xl mb-6 text-center font-mono text-red-500"/>
                <button onClick={dispararContingencia} className="w-full bg-red-600 py-5 rounded-2xl font-black uppercase text-xs flex justify-center items-center gap-3">
                   <Zap size={20}/> Acionar Matrix Local
                </button>
             </div>

             {/* WEB3 BLOCKCHAIN (FUSÃO DO ARQUIVO SOLTO) */}
             <div className="bg-gradient-to-br from-[#0d1117] to-[#020617] p-10 rounded-[3rem] border-2 border-cyan-500/30 shadow-[0_0_50px_rgba(6,182,212,0.1)]">
                <div className="flex items-center gap-3 mb-4">
                   <Globe className="text-cyan-400 animate-spin-slow" size={24}/>
                ...
                <p className="text-white text-[10px] uppercase font-black mb-10 leading-relaxed">
                   ...
                   Exige autorização via Carteira Digital (Owner).
                </p>
                <button 
                  onClick={handleSorteioOficialBlockchain}
                  disabled={loading}
                  className="w-full bg-cyan-600 hover:bg-cyan-500 py-6 rounded-2xl font-black uppercase text-xs shadow-xl flex justify-center items-center gap-3 transition-all"
                >
