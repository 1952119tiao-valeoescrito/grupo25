    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative">
      ...
      <main className="max-w-[1100px] mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        <section className="mb-10">
          <div style={{fontFamily:'Orbitron'}} className="text-4xl md:text-8xl font-black text-white drop-shadow-[0_0_15px_rgba(34,211,238,0.8)] tracking-tighter mb-2">{timer}</div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
             <h2 className="text-yellow-500 font-black text-[10px] uppercase mb-8 tracking-widest">Sua Malha Matrix 5x5</h2>
             <div className="bg-black/80 border border-slate-800 rounded-[2rem] p-4 md:p-10 mb-8">
                <div className="grid grid-cols-6 gap-1.5 md:gap-4 items-center">
                  {[0,1,2,3,4].map((i) => (
                    <div key={i} className="contents">
                      <span className="text-[8px] md:text-[10px] font-black text-cyan-500/40 text-right pr-1 font-elite">{i+1}º</span>
                      {[0,1,2,3,4].map((j) => (
             ...
             {!qrCode ? (
                <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                  <button onClick={gerarMalha} className="flex-1 bg-slate-800 p-4 rounded-2xl font-black text-[10px] uppercase border border-white/5 hover:bg-slate-700">Trocar Coordenadas</button>
                  <button onClick={handleConfirmar} disabled={loading} className="flex-1 bg-[#ea580c] p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg flex justify-center items-center gap-2">
             ...
             <p className="text-[10px] text-white/50 mt-6 font-bold uppercase tracking-widest italic">ID: {user.nome}</p>
          </div>

          <div className="space-y-6 text-left">
            <div className="bg-[#0d1117] border border-amber-500/30 p-6 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
               <div className="absolute top-0 right-0 bg-amber-500 text-black font-black text-[8px] px-3 py-1 rounded-bl-xl uppercase">ELITE</div>
               <h3 className="text-[10px] text-amber-500 mb-6 font-black uppercase font-elite tracking-widest">💰 Meu Lucro</h3>
            ...
            <div className="bg-[#0d1117] border border-white/5 p-6 rounded-[2.5rem] shadow-2xl">
               <h3 className="text-yellow-500 mb-6 uppercase font-bold text-[9px] tracking-widest font-elite">⚖️ Transparência Legal</h3>
        ...
        <div className="max-w-md mx-auto bg-[#0d1117] border border-emerald-500/30 p-8 rounded-[2.5rem] shadow-2xl mt-10">
            <h3 className="text-[10px] text-emerald-400 mb-4 uppercase tracking-widest font-black font-elite">🏆 Ranking Mimosinha</h3>
            <p className="text-slate-500 italic text-[11px] uppercase tracking-tighter animate-pulse">Sincronizando competidores...</p>
        </div>
      </main>

      <footer className="py-16 border-t border-white/5 opacity-20 text-center">
