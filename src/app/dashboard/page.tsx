...
export default function DashboardEliteFinal() {
  ...
  const [matriz, setMatriz] = useState([]); // Inicia vazia conforme solicitado
  ...
  // 1. Efeito Matrix Background (Z-Index 0)
  useEffect(() => {
    setMounted(true);
  ...
  const gerarMalha = () => {
    ...
    setMatriz(linhas);
    setQrCode(""); // Limpa QR anterior ao gerar nova malha
  };
  ...
  const handleConfirmarCertificado = async () => {
    ...
    try {
      ...
      if(data.qrCode) {
        ...
        alert("Erro ao gerar Pix. Verifique o token na Vercel.");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto items-stretch">
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl text-left backdrop-blur-md">
               ...
               <div className="space-y-4">
                  ...
                  <button className="w-full bg-cyan-700 p-5 rounded-2xl font-black text-xs uppercase shadow-lg hover:bg-cyan-600 transition-all">ACESSAR MATRIX</button>
               </div>
            </div>
            ...
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               ...
               <button onClick={handleGerarPix} disabled={loading || matriz.length === 0} className="w-full bg-slate-800 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 shadow-lg transition-all hover:bg-slate-700">
        ...
        {/* 4. BASE: MALHA MATRIX (COM RÓTULOS 1º AO 5º) E COLUNA DIREITA */}
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-20">
                ...
                <div className="flex flex-col gap-3">
                  {[0,1,2,3,4].map((i) => (
                    <div key={i} className="flex items-center gap-3">
                      <span className="text-[8px] md:text-[10px] font-black text-cyan-500/40 text-right w-20 uppercase italic">{i+1} PREMIO:</span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0d1117] border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
               <h3 className="text-yellow-500 mb-6 uppercase font-bold text-[9px] tracking-widest italic font-elite text-center italic text-white shadow-yellow-500/50">⚖️ Transparência Legal</h3>
