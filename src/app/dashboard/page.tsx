...
export default function DashboardElite() {
  ...
  // 01. BOTÃƒO ACESSAR MATRIX
  const handleAcessarMatrix = () => {
    alert("ConexÃ£o Matrix Estabelecida. UsuÃ¡rio: " + user?.nome);
  };
  ...
  const handleGerarPix = async () => {
    ...
    try {
      ...
      else { alert("MOTIVO DO ERRO: " + (data.error || "Resposta inválida da API")); }
    } catch (e) { alert("Erro de conexÃ£o"); }
    setLoading(false);
  };

  // 03. BOTÃƒO CONFIRMAR CERTIFICADO (LARANJA)
  const handleConfirmarCertificado = () => {
    if(!qrCode) return alert("Gere o Pix primeiro no Card 02!");
    localStorage.setItem('CERTIFICADO_G25', JSON.stringify({
        coords: matriz.flat(),
        usuario: user.nome,
        data: new Date().toLocaleString()
    }));
    router.push('/bilhete/imprimir');
  };
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <div className="w-full bg-black border-b border-cyan-500/30 py-3 overflow-hidden h-[45px] z-50 relative flex items-center">
        <div className="animate-marquee whitespace-nowrap text-cyan-400 font-black uppercase text-[11px] tracking-widest font-elite">
           ðŸš€ BEM-VINDO Ã€ MIMOSINHA BRASIL: SORTEIOS GRATUITOS NO MODO TREINO &nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp; ðŸ’¸ NO MODO REAL O PAGAMENTO Ã‰ AUTOMÃTICO VIA PIX
        </div>
      </div>
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center font-elite">
          ...
          <nav className="hidden md:flex gap-6 text-[9px] font-bold uppercase tracking-widest text-slate-400">
             <button onClick={()=>router.push('/meus-bilhetes')} className="hover:text-white transition-all text-xs">REGISTROS</button>
             <button onClick={()=>router.push('/resultados')} className="hover:text-white transition-all text-xs">RESULTADOS</button>
             <button onClick={()=>{localStorage.clear(); window.location.href='/';}} className="text-red-500 font-black text-xs uppercase">Sair</button>
          </nav>
          <div className="flex items-center gap-4">
             <span className="text-[9px] font-black text-yellow-500 border border-yellow-500/20 px-3 py-1 rounded-full uppercase italic">Acesso Restrito</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <section className="mb-10">
          ...
          <p className="text-[9px] text-yellow-500 font-bold uppercase tracking-widest italic text-center">SÃ¡bado Ã s 20:00hrs</p>
          <p className="text-cyan-400 font-bold uppercase tracking-[0.4em] text-[10px] text-center mt-4">Nossa produÃ§Ã£o 100% blockchain</p>
        </section>

        {/* 1. TOPO: CARDS INTER-BET E QUINA-BET */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 max-w-5xl mx-auto">
          <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl text-center backdrop-blur-md">
             <h3 className="text-2xl text-yellow-500 mb-2 font-black uppercase italic" style={{fontFamily:'Orbitron'}}>INTER-BET</h3>
             <p className="text-[10px] text-slate-400 mb-8 font-bold uppercase">Ganha com 5, 4, 3, 2 e atÃ© com 1 ponto apenas.</p>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/inter-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg hover:scale-105 transition-all">Acessar Site</button>
          </div>
          <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl text-center backdrop-blur-md">
             <h3 className="text-2xl text-cyan-400 mb-2 font-black uppercase italic" style={{fontFamily:'Orbitron'}}>QUINA-BET</h3>
             <p className="text-[10px] text-slate-400 mb-8 font-bold uppercase">Loteria clÃ¡ssica de 5 prognÃ³sticos.</p>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/quina-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg hover:scale-105 transition-all">Acessar Site</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto items-stretch">
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl text-left backdrop-blur-md">
               ...
               <div className="space-y-4">
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 text-slate-300 text-sm">{user.email}</div>
                  <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 text-slate-300 text-sm">********</div>
                  <button onClick={handleAcessarMatrix} className="w-full bg-cyan-700 hover:bg-cyan-600 p-5 rounded-2xl font-black text-xs uppercase shadow-lg transition-all">ACESSAR MATRIX</button>
               </div>
            </div>
            ...
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               <h3 className="text-[11px] text-cyan-400 mb-8 uppercase font-black" style={{fontFamily:'Orbitron'}}>02. CRÃ‰DITO (R$ 10)</h3>
               <div className="bg-white p-4 rounded-3xl mb-8 shadow-inner flex items-center justify-center">
                 {!qrCode ? (
                    <div className="w-32 h-32 bg-slate-100 flex items-center justify-center rounded-xl animate-pulse">
                        <Loader2 className="text-slate-300" />
                    </div>
                 ) : <QRCodeSVG value={qrCode} size={128} />}
               </div>
               <button onClick={handleGerarPix} disabled={loading} className="w-full bg-slate-800 hover:bg-slate-700 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 transition-all">
        ...
        <section className="mb-20">
           <h2 className="text-xl md:text-4xl font-black uppercase tracking-tighter mb-4 text-white">
             A ÃšNICA MATRIZ ONDE <span className="text-yellow-500 italic">ERRAR 24 VEZES</span> <br/>AINDA TE FAZ UM VENCEDOR.
           </h2>
        </section>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-20">
          <div className="lg:col-span-2 bg-[#0d1117] border border-cyan-500/30 p-6 md:p-10 rounded-[3rem] shadow-2xl">
                ...
                <div className="grid grid-cols-6 gap-1.5 md:gap-4 items-center">
                  {[0,1,2,3,4].map((i) => (
                    <div key={i} className="contents">
                      <span className="text-[8px] md:text-[10px] font-black text-cyan-500/40 text-right pr-1 italic font-elite">{i+1}Âº</span>
             ...
             <div className="flex gap-4 max-w-md mx-auto">
                ...
                <button onClick={handleConfirmarCertificado} className="flex-1 bg-[#ea580c] hover:bg-orange-500 p-4 rounded-2xl font-black text-[10px] uppercase shadow-lg font-elite transition-all">Confirmar Certificado</button>
             </div>
             <p className="text-[10px] text-white/30 text-center mt-6 uppercase font-bold italic tracking-widest text-center">Identificado: {user.nome}</p>
          </div>

          <div className="space-y-6">
            <div className="bg-[#0f172a] border border-amber-500/30 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
               <div className="absolute top-0 right-0 bg-amber-500 text-black font-black text-[8px] px-3 py-1 rounded-bl-xl uppercase italic">SÃ“CIO AFILIADO</div>
               <h3 className="text-[10px] text-amber-500 mb-6 font-black uppercase tracking-widest italic font-elite">ðŸ’° Meu Lucro</h3>
               <p className="text-[8px] text-slate-500 uppercase font-bold mb-1">Saldo DisponÃ­vel</p>
               <p style={{fontFamily:'Orbitron'}} className="text-3xl text-white font-black italic tracking-tighter font-elite">R$ 0,00</p>
               <button onClick={()=>alert("TransferÃªncia solicitada via Pix")} className="w-full bg-[#ea580c] hover:bg-orange-500 text-white p-4 rounded-xl text-[10px] font-black uppercase mt-6 shadow-lg transition-all font-elite">SACAR VIA PIX</button>
            </div>
            <div className="bg-[#0f172a] border border-emerald-500/30 p-8 rounded-[2.5rem] shadow-2xl text-center">
               <h3 className="text-[10px] text-emerald-400 mb-4 uppercase tracking-widest font-black font-elite flex items-center justify-center gap-2">ðŸ† Ranking Semanal</h3>
            ...
            <div className="bg-[#0d1117] border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
               <h3 className="text-yellow-500 mb-6 uppercase font-bold text-[9px] tracking-widest italic font-elite text-center">âš–ï¸ TransparÃªncia Legal</h3>
               <div className="space-y-4 text-[9px] font-bold text-slate-400 uppercase font-mono">
                  <div className="flex justify-between border-b border-white/5 pb-2"><span>PrÃªmio (43,35%)</span><span className="text-white">R$ 0,00</span></div>
                  ...
                  <div className="flex justify-between"><span>EducaÃ§Ã£o (9,26%)</span><span className="text-white">R$ 0,00</span></div>
               </div>
            </div>
          </div>
        </div>

        {/* --- NOVO: CARD DE CONTATO (MOVIDO DO PRINT 1) --- */}
        <div className="bg-[#0d1117]/80 border border-cyan-500/20 p-10 md:p-14 rounded-[4rem] mb-12 max-w-4xl mx-auto shadow-2xl backdrop-blur-md">
        ...
        <footer className="py-20 border-t border-white/5 opacity-30 text-center relative z-[200]">
           <p onClick={()=>{setClickCount(c=>c+1); if(clickCount>=4) router.push('/admin/central')}} className="text-[10px] font-black uppercase tracking-[0.5em] italic cursor-pointer select-none antialiased">
              Â© 2026 BET-GRUPO25 | PROTOCOLOS MATRIX PRO | BY NEON DATABASE
           </p>
        </footer>
      </main>
