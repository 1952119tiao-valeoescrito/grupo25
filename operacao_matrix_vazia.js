...
import { Trophy, RefreshCw, ChevronRight, Loader2, LogOut, Wallet, Scale } from 'lucide-react';
...
export default function DashboardElite() {
  ...
  const [matriz, setMatriz] = useState([]); // GARANTE QUE COMEÇA VAZIA
  ...
  // FUNÇÃO QUE GERA O PIX E ENTREGA O BILHETE
  const handleConfirmar = async () => {
    ...
    try {
      ...
      if(data.qrCode && data.ticketId) {
        // Salva os dados para o certificado ler
        ...
        // REGRA: LIMPA A MATRIX NA TELA
        setMatriz([]); 
        
        // REDIRECIONA PARA O BILHETE
        router.push('/bilhete/' + data.ticketId);
      } else {
        alert("Erro no banco ou no Pix. Verifique se a Rodada #1 foi criada.");
      }
    ...
    <div className="min-h-screen bg-[#010409] text-white font-sans overflow-x-hidden relative selection:bg-cyan-500/30">
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
             <span className="text-[10px] font-bold text-yellow-500 uppercase border border-yellow-500/20 px-4 py-1.5 rounded-full bg-yellow-500/10 italic">Olá, {user.nome.split(' ')[0]}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        <section className="mb-10">
          <div style={{fontFamily:'Orbitron'}} className="text-4xl md:text-9xl font-black text-white drop-shadow-[0_0_15px_rgba(34,211,238,0.8)] tracking-tighter mb-2 italic text-center">{timer}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 max-w-5xl mx-auto">
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-8 md:p-10 rounded-[3.5rem] shadow-2xl text-center">
             <h3 className="text-2xl text-yellow-500 mb-2 font-black uppercase italic" style={{fontFamily: 'Orbitron'}}>INTER-BET</h3>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/inter-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg mt-4">Acessar Site</button>
          </div>
          <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-8 md:p-10 rounded-[3.5rem] shadow-2xl text-center">
             <h3 className="text-2xl text-cyan-400 mb-2 font-black uppercase italic" style={{fontFamily: 'Orbitron'}}>QUINA-BET</h3>
             <button onClick={()=>window.open('https://blockchain-betbrasil.io/pt/quina-bet')} className="w-full bg-cyan-500 text-black py-4 rounded-3xl font-black text-[10px] uppercase shadow-lg mt-4">Acessar Site</button>
          </div>
        </div>

        {/* MALHA MATRIX */}
        <div className="bg-[#0f172a]/95 border border-cyan-500/20 p-6 md:p-12 rounded-[3.5rem] shadow-2xl mb-12 max-w-4xl mx-auto">
          <h2 className="text-yellow-500 font-bold text-xs uppercase mb-8 tracking-widest text-center" style={{fontFamily: 'Orbitron'}}>Sua Malha de Coordenadas Matrix 5x5</h2>
          <div className="bg-black/90 border border-slate-800 rounded-[2rem] p-4 md:p-10 mb-10">
            <div className="grid grid-cols-6 gap-2 md:gap-4 items-center">
              {[0,1,2,3,4].map((i) => (
                <div key={i} className="contents">
                  ...
                  {[0,1,2,3,4].map((j) => (
                    <div key={j} className="aspect-square bg-slate-900 border border-cyan-500/30 rounded-lg flex items-center justify-center text-[10px] md:text-sm font-black text-cyan-400 font-elite">
                      {matriz[i] ? matriz[i][j] : '--/--'}
                    </div>
                  ))}
                </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto">
            <button onClick={gerarMalha} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-4 rounded-2xl font-black uppercase text-[10px] border border-white/5 font-elite">Gerar Coordenadas</button>
...
console.log("✅ Dashboard Operacional: Matrix Vazia e Botão Laranja Religados!");