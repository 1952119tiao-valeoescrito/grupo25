...
export default function DashboardElite() {
  ...
  // 1. BLINDAGEM CONTRA TELA PRETA (MATA O ERRO DE EXCEÇÃO)
  useEffect(() => {
    setMounted(true);
      ...
      <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/10 h-20 flex items-center px-6">
        <div className="max-w-[1200px] mx-auto w-full flex justify-between items-center font-elite">
          ...
          <div className="flex items-center gap-4">
             <span className="text-[9px] font-black text-yellow-500 border border-yellow-500/20 px-3 py-1 rounded-full uppercase italic">Olá, {user?.nome?.split(' ')[0] || 'Acesso'}</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 text-center relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 max-w-5xl mx-auto items-stretch">
            ...
            <div className="bg-[#0f172a]/95 border border-cyan-500/30 p-10 rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center backdrop-blur-md">
               ...
               <button onClick={handleConfirmar} className="w-full bg-slate-800 p-4 rounded-xl text-white font-black text-[10px] uppercase border border-white/5 shadow-lg">Gerar Pix</button>
            </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start text-left mb-20">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0d1117] border border-white/10 p-8 rounded-[2.5rem] shadow-2xl text-[9px] font-bold text-slate-400 uppercase">
               <h3 className="text-yellow-500 mb-6 font-bold text-[10px] tracking-widest">⚖️ Transparência Legal</h3>
               <div className="space-y-4">
...
console.log("✅ DASHBOARD FINALIZADO COM SUCESSO!");